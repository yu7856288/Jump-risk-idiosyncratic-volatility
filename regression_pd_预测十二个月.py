...
def calcFirstRegression(y_var,x_vars):
    ...
    first_regression_data["r_rft"] = first_regression_data["r_rft"].shift(-12) ###预测12个月
    first_regression_data = first_regression_data.dropna(axis=0, how='all')  ###dropna返回一个df
    first_regression_data = first_regression_data.fillna(0)
...
if __name__ == "__main__":
    ...
    handler = logging.FileHandler('seven.log')
    logger.addHandler(handler)
    second_regression_coef_path="d:/pandas/result/test.txt"
    second_regression_result="d:/pandas/result/跳跃含iv_预测12个月_coef.xls"
    logger.info("########################################################################################")
    logger.info("predict 12 month")
