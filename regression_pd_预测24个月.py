...
def calcFirstRegression(y_var,x_vars):
    ...
    first_regression_data["r_rft"] = first_regression_data["r_rft"].shift(-24) ###预测6个月
...
if __name__ == "__main__":
    ...
    second_regression_result="d:/pandas/result/跳跃含iv_预测24个月_coef.xls"
    logger = logging.getLogger(__name__)
    logger.setLevel(level=logging.INFO)
    handler = logging.FileHandler('seven.log')
    logger.addHandler(handler)
    logger.info("########################################################################################")
    logger.info("predict 24 month")
